...
| `resume` | Резюме, CV | A4, IBM Plex Sans, teal, две колонки |
...
### Колонтитулы и номера страниц (journal, book)

Пользователь может попросить «убери полосочку сверху», «номер страницы внизу по
центру», «без колонтитулов» — это делается переопределением пресета:

- `show_header_footer: false` — убрать колонтитул целиком (и полоску, и номер);
- `header_rule: false` — оставить колонтитул, но убрать тонкую линию под ним;
- `page_num_position` — куда ставить номер страницы:
  `"auto"` (по умолчанию: журнал — сверху справа; книга — зеркально, на внешнем
  крае разворота), `"top-left"`, `"top-center"`, `"top-right"`, `"bottom-left"`,
  `"bottom-center"`, `"bottom-right"`, `"none"` (без номера).

```python
doc = create_document(
    title="Сказка о тихом лесе",
    template="book",
    preset_overrides={
        "header_rule": False,              # «убери полосочку»
        "page_num_position": "bottom-center",  # «номер снизу посередине»
    }
)
```

В academic_ru номер страницы зафиксирован ГОСТом (внизу по центру) и не меняется.

### Шаблон resume — конвенции

Резюме собирается из обычных секций по правилам:

- `title` документа = имя человека, `author` = должность/подзаголовок под именем;
- секция **1-го уровня** = рубрика (Experience, Projects, Skills…);
- секция **2-го уровня** = запись внутри последней рубрики; её заголовок парсится
  как `"Должность — Компания | даты"` (компания и даты опциональны):
  `add_section(doc_id, "MCP Engineer — Acme | 2024 — now", "- пункт\n- пункт", level=2)`;
- рубрики с названиями про **навыки/skills, образование/education, языки/languages,
  сертификаты, интересы** автоматически уходят в правый сайдбар; рубрика
  **Contact/Контакты** становится строкой контактов в шапке (каждый контакт
  с новой строки);
- содержимое рубрики Skills из одной строки через запятую рендерится чипами;
- первое изображение в любой секции = круглая аватарка в шапке;
- колонтитулов и номеров страниц в резюме нет.
